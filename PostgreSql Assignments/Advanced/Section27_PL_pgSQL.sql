#####################Section27:PL/pgSQL-SQL Procedural lang######################
...
-Must Assign Veriables:

Two ways to assign:-
 -PL standard: variable:=value
 -PstgreSQL: variable:=value
...
CREATE OR REPLACE FUNCTION suppliers_to_record_from()
RETURNS SETOF suupliers AS $$
BIGIN
...
SELECT * FROM suppliers_to_recorder_from()
...
1.RETURN QUERY querey;
2.RETURN NEXT  expression;
nither return from fun but append zero or more rows the fun's result set.
